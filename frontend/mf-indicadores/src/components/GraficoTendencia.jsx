...
function DotPersonalizado(props) {
  ...
  return <circle cx={cx} cy={cy} r={4.5} fill={isBelowThreshold ? '#dc2626' : '#1E5FA8'} stroke="#ffffff" strokeWidth={2} />;
}
...
function TooltipPersonalizado({ active, payload }) {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm shadow-lg">
      <p className="font-semibold text-slate-900">{point.fechaCompleta}</p>
      <p className="mt-1 text-slate-600">Valor: {point.valorFormateado}</p>
    </div>
  );
}
...
export default function GraficoTendencia({ kpiId, nombre }) {
  ...
  return (
    <div className="mt-5 space-y-4 rounded-3xl border border-slate-200 bg-slate-50 p-4" onClick={(event) => event.stopPropagation()}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.22em] text-slate-500">Tendencia</p>
          ...
          {[7, 30, 90].map((dias) => (
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-semibold transition-colors ${
                periodo === dias ? 'bg-[#1E5FA8] text-white' : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              {dias} días
            </button>
      ...
      <div className="h-[320px] rounded-3xl bg-white p-3 shadow-sm">
        {loading ? (
          <div className="flex h-full items-center justify-center text-sm font-medium text-slate-500">Cargando tendencia...</div>
        ) : error || !state.kpi ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={series} margin={{ top: 10, right: 16, left: 0, bottom: 10 }}>
              ...
              <ReferenceLine y={state.kpi.meta} stroke="#1E5FA8" strokeDasharray="5 5" label={{ value: 'Meta', position: 'insideTopRight', fill: '#1E5FA8', fontSize: 12 }} />
              <Line
                type="monotone"
                dataKey="valor"
                stroke="#1E5FA8"
                ...
                activeDot={{ r: 6, stroke: '#1E5FA8', strokeWidth: 2, fill: '#ffffff' }}
              />
            </LineChart>
          </ResponsiveContainer>
        )}
